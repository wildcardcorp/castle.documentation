...
.. code:: robotframework
   ...
   *** Test Cases ***
   ...
   Show display menu
       ...
       Click link  css=#Castle-contentmenu-display a

       Wait until element is visible
       ...  css=#Castle-contentmenu-display li.Castle-toolbar-submenu-header
...
Castle by default comes with these displays:
...
The basic list view functionality described above for folders fits the normal way we think of folders -- as containers of items -- but Castle adds a nice facility to set the view of a folder to be that of any single item contained within the folder.
...
Or, you can set it to a *"collection"*, which on its own is already a powerful content filter. One common use-case, and one that is used in a default Castle site, is to have a Folder called "News". In it, there are individual News items, but also a Collection which will sort all of these news items so the most recent one is shown first.
...
Setting the default view of a folder to be one of the items contained in that folder, is one of the most powerful features of Castle.
...
Castle will display a message in these cases, along the lines of "You are editing the default view of a container. If you wanted to edit the container itself, go *here*."
...
So, while the "Display" menu is one of the most powerful features of Castle, it is wise to note for yourself which was the previous view - before you change it - so you can quickly change it back when needed.
